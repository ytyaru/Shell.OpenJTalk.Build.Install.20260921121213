...
HTSVOICE_TSV = script_dir.parent / "0-html-to-tsv" / "htsvoice-akihiro0105.tsv"
DROPBOX_ID_TSV = script_dir / "dropbox-id.tsv"
...
def download(name_url_rows):
    download_dir = script_dir / "zip"
    download_dir.mkdir(parents=True, exist_ok=True)
    
    for char_name, direct_url in name_url_rows:
        # download_dir の中にファイル名を作る（Pathオブジェクトの結合）
        file_path = download_dir / f"{char_name}_1.0.zip"
        
        # -O にフルパスをそのまま渡す
        cmd = ["wget", "-O", file_path, direct_url]
